...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    ...
    //底盘电流环pid值
		...
		const static fp32 motor_position_pid[3]  = {M2006_MOTOR_POSITION_PID_KP,M2006_MOTOR_POSITION_PID_KI,M2006_MOTOR_POSITION_PID_KD};
		int i=0;
...
//    uint8_t i;
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        PID_init(&chassis_move_init->motor_speed_pid[i],   PID_POSITION,    motor_speed_pid  , M2006_MOTOR_SPEED_PID_MAX_OUT  , M2006_MOTOR_SPEED_PID_MAX_IOUT  );
		    PID_init(&chassis_move_init->chassis_angle_pid[i],PID_POSITION,motor_position_pid, M2006_MOTOR_POSITION_PID_MAX_OUT  , M2006_MOTOR_POSITION_PID_MAX_IOUT);
		}
...
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
    ...
    for (i = 0; i < 4; i++)
    {
			//  chassis_move_control_loop->motor_chassis[i].current_set=M2006_MOTOR_CURRENT_SET;
			 chassis_move_control_loop->motor_chassis[i].ecd_set=M2006_MOTOR_ECD_SET;
